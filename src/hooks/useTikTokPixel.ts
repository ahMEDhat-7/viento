...
export const initializeTikTokPixel = () => {
  ...
  if (!PIXEL_ID) {
    console.warn('⚠️ TikTok Pixel ID is not configured in environment variables');
    return;
  }
  
  if (window.ttq && isPixelReady) {
    console.log('✅ TikTok Pixel already initialized and ready');
    return;
  }
